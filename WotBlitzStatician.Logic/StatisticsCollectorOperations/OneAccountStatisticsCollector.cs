...
{
    public class OneAccountStatisticsCollector : IStatisticsCollector
    {
        private readonly ILifetimeScope _childScope;
        private readonly long _accountId;

        public OneAccountStatisticsCollector(ILifetimeScope childScope, long accountId)
        {
            _accountId = accountId;
            (_childScope, OperationsCount) = CreateOperationSteps(childScope);
        }
        ...
        private static (ILifetimeScope, int) CreateOperationSteps(ILifetimeScope lifetimeScope)
        {
            ...
            {
                builder.Register((c, p) => new ReadOneAccountInfoToCollect(
                    c.Resolve<IAccountDataAccessor>(),
