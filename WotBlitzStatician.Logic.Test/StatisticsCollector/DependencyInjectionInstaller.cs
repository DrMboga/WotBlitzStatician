...
namespace WotBlitzStatician.Logic.Test.StatisticsCollector
{
  public static class DependencyInjectionInstaller
  {
    ...
    public static void SetupLogger(this ContainerBuilder containerBuilder, ILoggerFactory loggerFactory)
    {
      containerBuilder.RegisterInstance(
        loggerFactory.CreateLogger<StatisticsCollectorEngine>())
        .As<ILogger<StatisticsCollectorEngine>>();
      containerBuilder.RegisterInstance(
        loggerFactory.CreateLogger<ProlongAccessTokenIfNeeded>())
        .As<ILogger<ProlongAccessTokenIfNeeded>>();
      containerBuilder.RegisterInstance(
        loggerFactory.CreateLogger<BlitzStaticianDictionary>())
        .As<ILogger<BlitzStaticianDictionary>>();
    }
  }
}